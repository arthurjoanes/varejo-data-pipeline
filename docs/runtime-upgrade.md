...
O primeiro build exige mais tempo, rede e espaço, pois recompila componentes. A manutenção dessas receitas é responsabilidade do projeto: cada atualização demanda revisar locks, repetir controles dirigidos e validar Spark/Delta, migração, demo, recuperação e scan integral. O CI reprova qualquer HIGH/CRITICAL, sem as exceções anteriores de perfil. O scan da imagem combinada passou esse gate e manteve o MEDIUM documentado. **A suíte integrada das reconstruções passou no snapshot pré-UI**; a validação após integrar a interface permanece separada e pendente.
