...
(async () => {
  ...
  try {
    ...
    for (const [file, view, width] of cases) {
      ...
      await page.goto(pathToFileURL(path.join(htmlDirectory, `${file}.html`)).href + '#' + view);
    ...
    fs.writeFileSync(path.join(evidence, 'accessibility.json'), JSON.stringify(output, null, 2) + '\n');
