...
(async () => {
  ...
  try {
    ...
    const output = { browser: await browser.version(), captured_at: new Date().toISOString(), results, scope: 'Automated WCAG-tagged checks on 10 offline views. Complements keyboard, responsive and contrast checks; not a complete WCAG or screen-reader audit.' };
    fs.writeFileSync(path.join(root, 'docs/evidence/interface-v3/accessibility.json'), JSON.stringify(output, null, 2) + '\n');
